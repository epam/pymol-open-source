...
void SceneSetStereo(PyMOLGlobals * G, int flag)
{
  ...
  CShaderMgr_Set_Reload_Bits(G, RELOAD_ALL_SHADERS | RELOAD_SHADERS_UPDATE_FOR_BACKGROUND);
}
                           ...
                           GridInfo * grid, int dynamic_pass)
{
  ...
  GL_DEBUG_BLOCK((std::string(__FUNCTION__ "(): ") + (-1 <= pass && pass <= 1 ? passNames[pass + 1] : "unknown") + " pass").c_str());
                 ...
                 int click_side, int force_copy, int just_background)
{
  ...
  if(G->HaveGUI && G->ValidContext) {
    ...
    if(pick || smp) {
      GL_DEBUG_BLOCK("// Render objects: pick || smp");
...
/* need to scissor this */ nHighBits = VLAGetSize(highBitVLA);
	...
	glShadeModel(SettingGetGlobal_b(G, cSetting_pick_shading) ? GL_FLAT : GL_SMOOTH);
      ...
      short offscreen = 0;
      /* STANDARD RENDERING */
      GL_DEBUG_BLOCK("// Render objects: standard");
